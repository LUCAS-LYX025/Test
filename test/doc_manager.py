        ...
        self.tool_docs = {
            ...
            "test_case_generator": {
                ...
                **核心功能:**
                ...
                - 📊 **灵活导出**：支持Excel和Markdown格式导出，适配不同测试管理需求
                - 📚 **历史追溯**：完整记录生成历史，支持快速重新加载和对比

                **操作流程:**
                1. **选择模型** - 根据需求选择最适合的大模型提供商
                2. **配置API** - 输入对应模型的API密钥和相关配置
                3. **输入需求** - 手动输入或使用预设示例模板
                4. **设置选项** - 选择测试用例风格和输出语言
                5. **生成用例** - AI智能分析并生成标准化测试用例
                6. **导出结果** - 支持Excel和Markdown格式导出
