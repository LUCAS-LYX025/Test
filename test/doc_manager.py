...
class ToolDocumentationManager:
    ...
    def __init__(self):
        ...
        **核心功能:**
        - 🤖 **AI智能生成**：基于阿里大模型自动分析需求并生成测试用例
        - 🖼️ **图片OCR识别**：支持上传图片自动提取需求文字
        - 📊 **多格式导出**：支持Excel格式导出，便于测试管理
        - 📚 **历史记录**：自动保存生成历史，支持快速重新加载

        **操作流程:**
        1. **配置API** - 输入阿里大模型API密钥和用例ID前缀
        2. **输入需求** - 直接输入需求描述或使用图片OCR识别
        3. **选择示例** - 可快速选择预设的示例需求模板
        4. **生成用例** - 点击生成按钮，AI自动分析并生成测试用例
        5. **导出结果** - 将生成的测试用例导出为Excel文件

        **功能模块详解:**
        - 🔑 **API配置**：设置阿里大模型访问密钥和用例编号规则
        - 🖼️ **图片OCR**：上传需求截图或文档图片，自动提取文字内容
        - 📝 **需求输入**：支持手动输入或使用预设示例模板
        - 🧠 **AI分析**：智能分析需求，生成全面覆盖的测试用例
        - 📊 **结果展示**：表格形式展示生成的测试用例和统计信息
        - 📤 **数据导出**：Excel格式导出，兼容主流测试管理工具
        """,
                "notes": """
        ### 重要注意事项
        ...
        **API配置说明:**
        - 🔐 **密钥安全**：妥善保管API密钥，避免泄露
        - 🌐 **网络要求**：确保网络通畅，能够访问阿里云API服务
        - 💰 **费用说明**：使用阿里大模型API可能会产生相应费用

        **图片OCR功能:**
        - 📷 **图片格式**：支持PNG、JPG、JPEG、BMP等常见格式
        - 🔍 **识别精度**：识别效果取决于图片质量和文字清晰度
        - 🇨🇳 **语言支持**：支持中英文混合识别，中文识别需要安装中文语言包
        - ⚡ **预处理**：自动进行灰度化和二值化处理提升识别率

        **测试用例生成:**
        - 🎯 **覆盖全面**：AI会自动考虑正常场景、异常场景和边界条件
        - 📋 **标准格式**：生成的用例包含用例ID、名称、前置条件、测试步骤、预期结果、优先级
        - ⚡ **智能分级**：根据测试重要性自动设置高、中、低优先级
        - 🔄 **可编辑性**：生成的结果支持进一步手动调整和优化

        **技术实现说明:**
        - **OCR引擎**：基于Tesseract OCR，支持多语言文字识别
        - **图像处理**：使用OpenCV进行图片预处理优化识别效果
        - **AI模型**：集成阿里通义千问大模型进行需求分析和用例生成
        - **数据导出**：使用openpyxl生成标准Excel格式文件

        **性能优化建议:**
        - 🚀 **需求描述**：提供越详细的需求描述，生成的用例越准确
        - 💾 **图片大小**：过大的图片会影响OCR处理速度，建议适当压缩
        - ⏱️ **生成时间**：复杂需求可能需要较长的AI处理时间
        - 📋 **分批处理**：超大需求建议拆分成多个功能点分别生成
        ...
        **质量保证措施:**
        - 🔍 **人工复核**：AI生成的用例建议人工复核确认
        - 📝 **补充完善**：根据实际测试经验补充AI未覆盖的场景
        - 🎯 **优先级调整**：根据项目实际情况调整用例执行优先级
        - 🔄 **持续优化**：根据测试结果反馈不断优化用例设计

        **开发注意事项:**
        - 🔧 **环境依赖**：需要安装Tesseract OCR系统组件
        - 📦 **Python库**：依赖opencv-python、pytesseract、openpyxl等库
        - 🌐 **网络访问**：需要能够访问阿里云API端点
        - 💾 **内存管理**：大图片处理时注意内存使用情况

        **温馨提示:**
        > AI生成的测试用例作为辅助工具，实际测试时请结合业务逻辑和测试经验进行完善。
        > 重要项目的测试用例建议由资深测试工程师进行最终评审。
        > 定期更新OCR和AI模型以获得更好的识别和生成效果。
        > 注意保护需求文档中的敏感信息，避免通过图片泄露。
        """
            }
        }
