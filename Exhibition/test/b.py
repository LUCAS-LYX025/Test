...
# 文本对比工具
    ...
    with col1:
        st.subheader("原始文本")
        text1 = st.text_area("", height=300, key="text1")

    with col2:
        st.subheader("对比文本")
        text2 = st.text_area("", height=300, key="text2")
    ...
    with col2:
        if st.button("清空所有内容"):
            # 使用session_state来清空内容
            if 'clear_text' not in st.session_state:
                st.session_state.clear_text = True
            else:
                st.session_state.clear_text = not st.session_state.clear_text
            st.rerun()
...
# JSON数据对比工具
    ...
    with col1:
        st.subheader("JSON 1")
        json1 = st.text_area("", height=300, key="json1", placeholder='{"name": "John", "age": 30}')

    with col2:
        st.subheader("JSON 2")
        json2 = st.text_area("", height=300, key="json2", placeholder='{"name": "Jane", "age": 25}')
    ...
    with col2:
        ...
        if st.button("清空"):
            ...
            if 'clear_json' not in st.session_state:
                ...
                st.session_state.clear_json = not st.session_state.clear_json
            st.rerun()
